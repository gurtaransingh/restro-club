...
import { ChefHat, Clock, CheckCircle2, AlertTriangle, Sparkles, LogIn } from 'lucide-react';

export default function KitchenPage() {
  const { orders, updateOrderStatus, activeLocation, isAuthenticated, isChef, isSuperAdmin } = useApp();

  if (!isAuthenticated || (!isChef && !isSuperAdmin)) {
    return (
      <div className="min-h-screen bg-[#FAF8F3] text-[#1E241D] py-20 px-4 text-center flex flex-col items-center justify-center">
        <div className="max-w-md w-full bg-white border border-[#E5DEC9] rounded-3xl p-8 shadow-sm space-y-5">
          <div className="w-14 h-14 bg-[#3E4A38]/10 text-[#3E4A38] rounded-2xl flex items-center justify-center mx-auto">
            <ChefHat className="w-7 h-7" />
          </div>
          <h1 className="font-serif italic text-2xl font-bold text-[#1E241D]">Kitchen Display System Access</h1>
          <p className="text-xs text-[#5C554E]">This portal is reserved for Executive Chefs and Culinary Station Managers.</p>
          <div className="pt-2">
            <Link
              href="/login"
              className="w-full py-3 bg-[#3E4A38] text-white font-bold text-xs uppercase tracking-wider rounded-xl hover:bg-[#2F392B] transition-all flex items-center justify-center gap-2"
            >
              <LogIn className="w-4 h-4" />
              <span>Sign In as Chef (c1 / c1)</span>
            </Link>
          </div>
        </div>
      </div>
