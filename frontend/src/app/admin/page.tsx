...
export default function AdminPage() {
    ...
    return (
      <div className="min-h-screen bg-[#FAF8F3] text-[#1E241D] py-20 px-4 text-center flex flex-col items-center justify-center">
        <div className="max-w-md w-full bg-white border border-[#E5DEC9] rounded-3xl p-8 shadow-sm space-y-5">
          ...
          <h1 className="font-serif italic text-2xl font-bold text-[#1E241D]">Super Admin Hub Protected</h1>
          <p className="text-xs text-[#5C554E]">Requires Super Administrator (admin / admin) or General Manager privileges.</p>
          <div className="pt-2">
            <Link
              href="/login"
              className="w-full py-3.5 bg-[#8C5A3C] text-white font-bold text-xs uppercase tracking-wider rounded-xl hover:bg-[#73482E] transition-all flex items-center justify-center gap-2"
            >
  ...
  const adminModules = [
    ...
    },
    {
      ...
      count: `${locations.length} Operating Hubs`,
    ...
    },
    {
      ...
      count: `${orders.length} Orders Managed`,
      link: '/kitchen',
    ...
    },
    {
      ...
      desc: 'Staff rosters, monthly salary disbursements (Levels 0–50), KYC verification and designations.',
