...
export const SportsPage: React.FC = () => {
  const { facilities, activeLocation } = useApp();
  ...
  const filteredFacilities = facilities.filter((fac) => {
    if (fac.locationId !== activeLocation.id) return false;
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white pb-24">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-20">
        <div className="bg-[#141414] p-3 border border-white/10 rounded-2xl shadow-xl flex items-center gap-2 overflow-x-auto no-scrollbar">
          {categories.map((cat) => (
            <button
              key={cat}
              ...
              <div>
                <div className="relative h-56 overflow-hidden bg-[#0A0A0A]">
                  ...
                  <span className="absolute bottom-3 right-3 bg-[#0A0A0A] border border-white/20 text-white font-serif italic font-bold text-sm px-3.5 py-1 rounded-full shadow-lg">
                    {fac.memberPrice === 0 ? 'MEMBER FREE' : `₹${fac.memberPrice}`}
                  </span>
                </div>
